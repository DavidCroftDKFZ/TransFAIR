...
public class CSV_Mapping extends ID_Mapping {
  ...
  public void read_mappings()
      throws IOException, CsvException { // TODO: Automatically call this after
    ...
    while ((row = reader.readNext()) != null) {
      ...
      // //use indices of row as a row could be shorter than the number of domains e.g. because
      // there are no values in the last domain
      for (int a = 0; a < row.length; a++) {
        for (int b = a + 1; b < row.length; b++) {
          if (!row[a].equals("")
              && !row[b].equals("")) { // Check whether there is a value und each of the two domains
